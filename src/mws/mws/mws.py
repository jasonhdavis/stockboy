...
class Products(MWS):
    ...
    def get_my_fee_estimate(self, marketplaceid, asin, price, currencycode, isamazonfulfilled):
            data = dict(Action='GetMyFeesEstimate')
            data.update(utils.enumerate_param('FeesEstimateRequestList.FeesEstimateRequest.1.Identifier.', asin))
            data.update(utils.enumerate_param('FeesEstimateRequestList.FeesEstimateRequest.1.IdType.', 'ASIN'))
            data.update(utils.enumerate_param('FeesEstimateRequestList.FeesEstimateRequest.1.IdValue.', asin))
            data.update(utils.enumerate_param('FeesEstimateRequestList.FeesEstimateRequest.1.MarketplaceId.', marketplaceid))
            data.update(utils.enumerate_param('FeesEstimateRequestList.FeesEstimateRequest.1.PriceToEstimateFees.ListingPrice.Amount', price))
            data.update(utils.enumerate_param('FeesEstimateRequestList.FeesEstimateRequest.1.PriceToEstimateFees.ListingPrice.CurrencyCode', currencycode))
            data.update(utils.enumerate_param('FeesEstimateRequestList.FeesEstimateRequest.1.IsAmazonFulfilled.', isamazonfulfilled))
            return self.make_request(data)
